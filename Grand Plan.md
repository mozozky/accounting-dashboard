...
7. Auth callback (`/auth/callback`) → `exchangeCodeForSession` → `ensureUserProfile` → assign role via RPC

### Security
- `/signup` dilindungi invite code (fail-closed)
...
### Timezone
Semua kalkulasi tanggal (overdue, due this week) menggunakan **Asia/Jakarta (WIB)** via `src/lib/utils/date.ts`. Bukan UTC — penting karena Vercel server jalan di UTC.
...
- List periods per task type per bulan
...
- Info banner: "Mengubah template di sini hanya berlaku untuk period yang di-generate berikutnya — edit period yang sudah ada langsung di halaman period task-nya."
...
│   │   ├── MonthSwitcher.tsx       # Navigasi bulan (amber kalau off-month)
...
- **RLS masih broad** — semua tabel pakai `authenticated full access`. Guard saat ini di level server action. Untuk keamanan lebih ketat, perlu row-level ownership policy di DB.
- **Invite flow** — saat ini pakai invite code manual. Bisa ditingkatkan ke proper invite email flow.
- **"Save to Template"** — checklist yang ditambah manual di period page tidak carry forward ke bulan baru. Harus tambah via Settings → Stage Templates untuk recurring. Fitur shortcut "Save to Template" dari period page belum dibangun.
- **Batch period generation** masih sequential per client → kandidat optimasi kalau klien sangat banyak.
